...
repositoryUrl: https://github.com/hkateu/MyHttp4sCode/tree/main/oauth/src/main
tags: [http4s, scala]
...
When handling configuration with Ciris, values are passed around as ConfigValues. Here's how [the official Ciris website](https://cir.is/docs/configurations) defines a ConfigValue:

<dl>
  <dt>ConfigValue</dt>
  <dd>
    The central concept in the library. It represents a single configuration
    value or a composition of multiple values.
  </dd>
</dl>
...
`client.expect` could fail for several reasons and will return an appropriate status code depending on the reason, however, we'll handle these errors in the coming section.

:::

Since this function is polymorphic in `F`, we'll need to bind an implicit `Async` from cats effect for this function to work. All the functions in this application will be polymorphic in `F` to allow for the use of any effect type, not necessarily `IO`.
...
We have learned about OAuth, and how to connect an application to another using this framework protocol. Once you are familiar with the OAuth [flow](https://auth0.com/docs/get-started/authentication-and-authorization-flow/which-oauth-2-0-flow-should-i-use), implementing it becomes easier.

:::tip

For more information on GitHub's web application flow, check out this [link](https://docs.github.com/en/apps/oauth-apps/building-oauth-apps/authorizing-oauth-apps).

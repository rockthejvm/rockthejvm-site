...
<CautionAdmonition>
  `client.expect` could fail for several reasons and will return an appropriate
  status code depending on the reason, however, we'll handle these errors in the
  coming section.
</CautionAdmonition>
...
<TipAdmonition>
  For more information on GitHub's web application flow, check out this

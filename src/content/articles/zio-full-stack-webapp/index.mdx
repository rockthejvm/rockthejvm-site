...
This article showcases Olivier's template of a Scala full-stack web application with ZIO and Laminar, after he completed the giant [ZIO Rite of Passage](/courses/zio-rite-of-passage) course on Rock the JVM. The course teaches you to build a full-blown product from scratch with the principles outlined in the article. If this sounds exciting, check it out [here](/courses/zio-rite-of-passage).
...
<table>
  <tr>
   ...
   <ul>
    ...
    <li>Sttp client leverages the same Tapir endpoint definitions</li>
   </ul>
...
A library for asynchronous and concurrent programming in Scala. There's a lot to say about it and Rock the JVM teaches this in detail in a [dedicated ZIO course](/courses/zio), but we'll cover the essentials below.
...
- Those two effects combine in a for comprehension **(3)** to produce a new ZIO **(4)** that accumulate the dependencies: ZIO[**FlywayService & PersonService & Server**,…]
...
`RIO` is just a type alias where error are Throwable:
...
Many more patterns to discover in the [ZIO rite of passage](/courses/zio-rite-of-passage) course.

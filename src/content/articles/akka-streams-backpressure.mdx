---
category: guide
excerpt: Discover how Akka Streams implements backpressure, a key component of the Reactive Streams specification, in this detailed demonstration
publishedDate: 2021-11-21
tags: [akka, akkaStreams]
title: Akka Streams Backpressure Explained
...
This article discusses one of the most important aspects of a reactive system using Akka Streams.
...
We discuss this in detail in the [Akka Streams](/courses/akka-pekko-streams) course, but to recap really quickly:
...
1. In a snap, the sink receives an element, but it takes 1 second to process it, so it will send a _backpressure signal_ upstream.
1. During that time, the flow will attempt to keep the throughput of the source, and _buffer 16 elements_ internally.
1. Once the flow's buffer is full, it will stop receiving new elements.
1. Once per second, the sink will continue to receive an element and print it to the console (the second, slow batch).
1. After 8 elements, the flow's buffer becomes half-empty. It will then resume the source and print 8 more elements in a burst, until its buffer is full again.
1. The slow sink will keep doing its thing.
1. After 8 more elements, the flow's buffer becomes half-empty again, which will resume the source.
1. And so on.
...
### Backpressure: Slow Down the Stream

...
Question: what functional "thing" is invocable like a function, but _not on all arguments_?
...
Same idea is applicable to Maps. Maps are "invocable" objects on a key, returning a value (if it exists) or throwing an exception (if it doesn't). What's that again? PartialFunction!

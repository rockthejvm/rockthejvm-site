---
category: guide
excerpt: "Level 90 of type-level programming: implementing a real sorting algorithm on lists as types"
...
1. Partitioning a one-element list results in that same list on the "left" and the empty list on the "right".
1. Considering a list with at least two elements `P :: N :: T` (pivot, next number and tail of the list), we'll need to assume a recursive partitioning of `T` into a "left" `L` (with elements smaller than the pivot) and a "right" `R` (with elements >= the pivot). Now we need to decide where `N` is going to stay.

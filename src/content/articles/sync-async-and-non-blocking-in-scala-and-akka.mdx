...
However, even in this example we have a drawback: we aren't returning any meaningful value out of the interaction. To solve that, we could return a Future which the actor might complete manually. Check out the [controllable Futures article](/articles/controllable-futures-in-scala) for details into the reasoning.
...
Now that you understand the trade-offs between synchronous, asynchronous, blocking and non-blocking computation, you can make informed decisions about how to structure your code.

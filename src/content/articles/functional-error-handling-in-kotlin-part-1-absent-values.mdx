...
However, working with pattern matching is only sometimes very convenient. A lot of time, we need to transform and combine different `Option` values. As in Scala, the `Option` type is a [monad](/an-introduction-to-monads-in-scala), so we can use the `map` and `flatMap` functions to transform and combine `Option` values. Let's see an example.

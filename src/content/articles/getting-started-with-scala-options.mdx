...
excerpt: "Scala Options are among the first concepts we encounter: Discover what they do, why they're useful, and their importance in programming"
publishedDate: 2022-06-02
tags: [scala, typeSystem]
title: Getting Started with Scala Options
...
If you're just getting started with Scala and Options seem hard to understand, this article is for you.

## Introduction

We'll introduce Scala Options, a seemingly contrived data structure, but we'll see in this post that it's extremely powerful and useful in its simplicity.

We teach Options in-depth in the [Scala Essentials](/courses/scala-essentials) course, along with examples and exercises, if you want to check it out. In this article, we'll introduce Options, show you how they work, and _why_ they exist.
...
However, in this case, the `flatMap` method doesn't have the _meaning_ of concatenation (as was the case for lists), because here there's either a single value, or nothing. For Option, the `flatMap` method is a way of _chaining_ computations that may either produce a value or nothing. This general idea of sequencing computations is [immensely powerful](/articles/an-introduction-to-monads-in-scala).
...
## Resisting the Temptation to Retrieve Option Values

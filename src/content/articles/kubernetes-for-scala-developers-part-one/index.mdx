...
title: "Deploying a Scala Application on Kubernetes - part 1"
...
### Scala application to deploy

#### Endpoints and application versions to deploy
In this article we'll deploy a Scala cars parking management application built with Play Framework uses an in-memory database
for simplicity.
The Docker images that we'll be using are:
`pfpw:20.20.31` and `pfpw:20.20.32` publicly available on Docker Hub. It exposes two api's that we can test:
* `/insertNewCar` to insert a new car in as a json object with the attributes: (year: Int, brand: String, maxSpeed: Double) with
...
* **Scala Play framework application built(`build.sbt`)**
```scala
name := """pfpw"""
...
//TwirlKeys.templateImports += "rockthejvm.controllers._"

// Adds additional packages into conf/routes
// play.sbt.routes.RoutesKeys.routesImport += "rockthejvm.binders._"
````

* **Scala code for simple cars parking management**
```scala 3
...
* **Memory workload code used to create surge in memory usage**

This code uses the `java.lang.Runtime` API to perform operations on the memory.
```scala 3
  /**
...
* **Code use to create a surge in CPU usage**

It's an incremental tail-recursive fibonaci sequence in the sense to
the value of the order `n` it goes from the value for `0` and `1`, and increment until it get the value for the order
`n-2` and `n-1` and from there it derives the value for the order `n` by doing `f(n) = f(n -1) + f(n-2)`.
...
* **Configurations in `application.conf` file**
```text
...
For the development environment multiple solutions exist, but we'll use Minikube as our local kubernetes. It is a
...
open a terminal at the root of that folder). The name of the file doesn't matter only its content matter. To test every
Kubernetes configuration in this article, you'll replace the content of the `workload-api.yaml` with the configuration
you want to test. `kubectl` is the Kubernetes client we use to send our request to the API-Server with is the front end
of any Kubernetes cluster. It is the element to which any new change on the cluster is sent to, and it collaborates with
other component to achieve the expected result if possible. To have a view of the Kubernetes internals we can run in a
separate terminal the commands `minikube addons enable dashboard` and `minikube addons enable metrics-server` that will
install the needed tools to visualize the activity on the cluster. We can then run the command `minikube dashboard` which
will open a web page in our default browser to see all the objects running on the cluster.
...
Explanations:
* **API Version(`apiVersion`) and kind(`Kind`)**: The first two lines specify the Kubernetes Pods API version, which is crucial for Kubernetes
...
* **Label Matching Strategy(`spec.elector.matchLabels`)**: Define precise label criteria to match and manage Pods, ensuring
that the ReplicaSet targets the correct set of containers across the Kubernetes environment. In this case pod having the labels
...
* call the endpoints `192.168.49.2:30123/listOfCars` and `192.168.49.2:30123/insertNewCar`
...
the core Ingress specification. `k8splayworkload.com` is the fully qualified domain name of a
...
To access the service trough the nginx ingress controller open your `/etc/hosts` in a linux machine(or corresponding settings on
...
The endpoints(`/listOfCars`, `/insertNewCar`) indicated at the beginning for cars parking management can be tested as indicated
using `k8splayworkload.com/listOfCars` and `k8splayworkload.com/insertNewCar` and the result is something like
...
Deployments are the actual project managers of Kubernetes. They handle updates, rollbacks, and ensure smooth transitions
...
In this article we have discovered the basic components of a Kubernetes cluster. how to create, observe, update and delete them.
The next part will explore Automatic creation and management of Kubernetes cluster using JenKins X and

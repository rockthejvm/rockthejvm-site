...
ENDPOINT="http://k8splayworkload.com/generateNewCars?n=1&t=2"  # Target HTTP endpoint
# Calling this end point will create a high asynchronous computation workload in the backend.
CALLS_PER_MINUTE=15                # Number of calls per minute
TOTAL_MINUTES=4                    # Total duration in minutes
...
echo "Mode: Synchronous (waiting for each response)"
echo "----------------------------------------"
...
while [ $(date +%s) -lt $END_TIME ]; do
  # Make HTTP POST request and wait for response
  REQUEST_START=$(date +%s.%N)

  HTTP_STATUS=$(curl -s -o /dev/null -w "%{http_code}" -X POST "$ENDPOINT")

  REQUEST_END=$(date +%s.%N)
  REQUEST_TIME=$(bc <<< "$REQUEST_END - $REQUEST_START")
  TOTAL_RESPONSE_TIME=$(bc <<< "$TOTAL_RESPONSE_TIME + $REQUEST_TIME")
  ...
  if [ $((COUNTER % 10)) -eq 0 ]; then
    ...
    AVG_RESPONSE_TIME=$(bc <<< "scale=4; $TOTAL_RESPONSE_TIME / $COUNTER")

    echo "Progress: $COUNTER requests sent ($ELAPSED_MINUTES minutes elapsed, $PERCENT_COMPLETE% complete)"
    echo "  Success: $SUCCESS_COUNT, Failed: $FAILED_COUNT, Avg response time: ${AVG_RESPONSE_TIME}s"
  fi

  # Calculate time to sleep (accounting for request time)
  ACTUAL_SLEEP_TIME=$(bc <<< "$SLEEP_TIME - $REQUEST_TIME")

  # Only sleep if the value is positive
  if (( $(bc <<< "$ACTUAL_SLEEP_TIME > 0") )); then
...
echo "Failed responses: $FAILED_COUNT"
echo "Success rate: $(bc <<< "scale=2; ($SUCCESS_COUNT / $COUNTER) * 100")%"
...
it progressively changes old instances(`pfpw:20.20.29`) with new instances having the new version/release(`pfpw:20.20.30`). Doing this there will be no time when
the service will not be available. To apply a new version run this command:
`kubectl set image -f workload-api.yml workload-api=aidocking/pfpw:20.20.30`

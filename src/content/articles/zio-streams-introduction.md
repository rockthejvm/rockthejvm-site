...
excerpt: "Unlock the Power of ZIO Streams: Your Comprehensive Guide to a Key ZIO Ecosystem Abstraction"
publishedDate: 2022-08-10
tags: [scala, zio, zio-streams]
...
If you're interested in learning the ZIO core library, check out the Rock the JVM [ZIO course](https://rockthejvm.com/courses/zio).
...
1. Remove any punctuation, in case the last work in the sentence is tag - as
   well as to remove the `#`.
1. Convert our parsed tags to lowercase
1. Put them all in a `Set[String]` to avoid duplicates.
...
1. Add a link to every `#tag`, which takes us to a special page on our blog that

...
Inlining code is often great, because it's very clear which code will be executed and which will not. Inlining code can also help the compiler infer _more precise_ types, because if a method is declared to return a general type, inlining might go on a path that returns a more concrete type. Regular methods may not make this inferrence, even though we as programmers can clearly tell:

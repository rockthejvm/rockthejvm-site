...
Now we'll need to start sending HTTP requests. I'll use the exact HTTP API I'm using for the blog: [http://markup.su/highlighter/api](http://markup.su/highlighter/api). The API says it needs GET or POST requests to /api/highlighter, with the parameters "language", "theme" and "source" in a request with content type application/x-www-form-urlencoded.
...
If you want to practice sending HTTP requests as an exercise, you can use [https://jsonplaceholder.typicode.com](https://jsonplaceholder.typicode.com) for dummy APIs, the same principle applies.

## Conclusion

This concludes our 5-minute tutorial on sending HTTP requests with Scala and Akka HTTP. I hope you found it useful.

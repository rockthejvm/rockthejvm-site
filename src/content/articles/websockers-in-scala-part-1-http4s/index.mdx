...
Within the `HttpRoutes.of[F]{}` method, we use the `StaticFile.fromPath()` method to which we pass an fs2 `Path` with the relative path to `chat.html` and a `Some(request)` as arguments then call `getOrElseF(NotFound())` that sends a 404 error code in case the file isn't found.
...
In this section, we'll learn how to parse the input messages we receive from the WebSocket. Create `InputMessage.scala` in `src/main/scala/rockthejvm/websockets/InputMessage.scala` and add the following code:
...
Next, we implement `commandParser` that returns a `Parser[TextCommand]`, the `Parser` type comes from cats-parse. Furthermore, `Parser[TextCommand]` means that whatever is parsed through this parser should return a `TextCommand` on success.
...
Finally, we call `wsb.build()` with custom `send()` and `receive()` functions. We'll see how these are implemented starting with the latter:

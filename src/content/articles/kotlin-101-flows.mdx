...
category: guide
excerpt: "This article delves into Kotlin Flows: a crucial reactive data structure in Kotlin Coroutines that, once discovered, becomes indispensable"
publishedDate: 2024-04-10
tags: [concurrency, coroutines, kotlin]
title: "Kotlin 101: Flows Quickly Explained"
...
In the article [Kotlin 101: Coroutines Quickly Explained](/articles/kotlin-101-coroutines), we saw how to use Kotlin Coroutines to write asynchronous code in a more natural and readable way. This article will focus on another crucial concept in Kotlin Coroutines: Kotlin flows. Flows are a data structure you didn't know, but you can't live without them once you know them. So, without further ado, let's dive into Kotlin Flows.

<TipAdmonition>
  If you need to become proficient in Kotlin **quickly** and with thousands of
  lines of code and a project under your belt, you'll love [Kotlin
...
Kotlin flows can be compared to the Typelevel FS2 library in Scala. We already discussed FS2 in the article [FS2: More Than Functional Streaming in Scala](/articles/fs2-more-than-functional-streaming-in-scala), and to help developers who are familiar with FS2 understand the concept of Kotlin flows, we'll use the same examples as in the FS2 article.
...
Now, the flow is collected inside a dedicated coroutine spawned by the `launch` coroutine builder (if you want to deep dive into the coroutines world, please refer to the article [Kotlin 101: Coroutines Quickly Explained](/articles/kotlin-101-coroutines)). The program will not wait for the whole collection of the flow to complete before printing the string "After Zack Snyder's Justice League". The output of the program is:
...
**The `flatMapMerge` function is handy when dealing with I/O operations on a collection of information**. We can set the concurrency level to the number of available processors to maximize the program's performance or even to fine-tune the maximum level of resources we want to use. Another approach is using the [`async` coroutine builder](/articles/kotlin-101-coroutines#the-async-builder) for each value in the collection:
...
If you have any questions or feedback, please let us know. We're always happy to hear from you. If this article was in any way too difficult and you need to become good at Kotlin as fast as possible, we think you'll love the [Kotlin Essentials course](/courses/kotlin-essentials).

## Appendix: How Flows Work

---
category: guide
excerpt: Akka, Cats, and Cassandra in a larger Scala project integrating multiple pieces of the Scala ecosystem
publishedDate: 2022-04-01
repositoryUrl: https://github.com/rockthejvm/akka-cassandra-demo
...
<NoteAdmonition>
  This mini-project is a collaboration between me (Daniel) and [Riccardo
  Cardin](/authors/riccardo-cardin), one of the prominent Rock the JVM alumni.
  Big thanks to Riccardo for contributing with most of the code that ended up in
  this article and on camera.
</NoteAdmonition>
...
- The [Akka Typed course](/courses/akka-pekko-essentials)
- The [Akka HTTP course](/courses/akka-pekko-http)
- The [Cats course](/courses/cats)
...
![Project architecture diagram showing Akka actors being used with Cassandra](images/project-architecture-diagram.png)

We're going to split the work on this application in 4 parts:

1. The bank account actor, using Akka Persistence
1. The "main" bank actor, also using Akka Persistence
1. The HTTP server with its REST API, using Akka HTTP
1. The data validation, using Cats

## The Bank Accounts

<aside>
  <MyYouTube id="PPIPGzrc2wo" />
</aside>
...
1. The events it will store in Cassandra
1. The structures to manage its internal state
1. The responses it may send to the main bank actor
...
<WarningAdmonition>
  **Please don't use the `Double` type to manage money**. The floating point
  standard cannot fully represent tenths and hundredths and can give (very)
  slightly incorrect results with multiplication and division in certain cases
  as a result. For our convenience in this project, we'll use `Double`.
</WarningAdmonition>
...
### The Bank Account Persistent Actor
...
1. Its state, which can change over time
1. Its message handler, aka _command_ handler
1. Its _event_ handler, i.e. what the actor does after storing an event to the persistent store, or _restoring_ an event after failure
...
<CautionAdmonition>
  This same handler will be invoked both after the event is stored, and if the
  actor/application crashes and the actor is restarted: in such a case, the
  actor queries Cassandra for all events tied to its persistence ID and replays
  all events in sequence by invoking the `eventHandler` on each of them in turn,
  to recreate its latest state before crash/shutdown.
</CautionAdmonition>
...
### Creating a Bank Account in the Akka HTTP Server
...
1. Send it a `CreateBankAccount` _command_[^1]
1. Parse its reply
1. Send back an HTTP response
...
[^1]: It's different from the HTTP request.
...
This bit is optional, because the application should work at this point. However, it's worth making our mini-bank a bit more robust in the face of malformed requests, and we can do this with the Cats validation capabilities. We talk about data validation and the Validated type in detail in the [Cats course](/courses/cats), but here we're not going to need too much.
...
We show in the [Advanced Scala course](/courses/advanced-scala) that when we have a single implicit behavior that makes sense for a type, we should place that implicit value in the companion of that type. In our case, we'll place the implicit type class instances in the companion objects of these requests:
...
- If the result is invalid, return an HTTP response with a `FailureResponse` aggregating _all_ the errors that were discovered

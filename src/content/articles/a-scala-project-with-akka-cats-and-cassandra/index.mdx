...
This mini-project is a collaboration between me (Daniel) and [Riccardo Cardin](/authors/riccardo-cardin), one of the prominent Rock the JVM alumni. Big thanks to Riccardo for contributing with most of the code that ended up in this article and on camera.
...
**Please don't use the `Double` type to manage money**. The floating point standard cannot fully represent tenths and hundredths and can give (very) slightly incorrect results with multiplication and division in certain cases as a result. For our convenience in this project, we'll use `Double`.

...
You can see the setup for the post in <a href={`${
// @ts-ignore
    frontmatter.repositoryUrl}/blob/master/setup.scala`}>this</a> file and follow along using the [Scala CLI](https://scala-cli.virtuslab.org/) tool.
...
The full code for the `Encoder` can be found in the accompanying <a href={`${
// @ts-ignore
    frontmatter.repositoryUrl}/blob/master/flat_concrete.scala`}>repo</a>.
...
The full code for the `Decoder` and `Codec` can be found in the accompanying <a href={`${
// @ts-ignore
    frontmatter.repositoryUrl}/blob/master/flat_concrete.scala`}>repo</a>.
...
The full code for this part can be found <a href={`${
// @ts-ignore
    frontmatter.repositoryUrl}/blob/master/flat_generic.scala`}>here</a>.
...
[^implementedInTheRepo]:
    You can find the solution to this exercise in the <a href={`${
// @ts-ignore
frontmatter.repositoryUrl}/blob/master/flat_modular.scala`}>repo</a>.
...
The full code for this part can be found in the <a href={`${
// @ts-ignore
    frontmatter.repositoryUrl}/blob/master/flat_modular.scala`}>repo</a>.
...
<WarningAdmonition>
    This is by far the most incomprehensible part of this post as well as the most speculative. I hope we make it out okay...
</WarningAdmonition>
...
[^implicitLabelling]:
    If you're curious what that would look like, I've implemented the same logic using implicits in the <a href={`${
// @ts-ignore
frontmatter.repositoryUrl}`}>repo</a>.
...
The full code for this part can be found in the <a href={`${
// @ts-ignore
    frontmatter.repositoryUrl}/blob/master/labelling.scala`}>repo</a>.
...
The code we'll be writing has a straightforward value-level equivalent, which you can find <a href={`${
// @ts-ignore
    frontmatter.repositoryUrl}/blob/master/value_level_find_duplicates.scala`}>here</a>[^notTheBest]. Feel free to take a look if you want a more precise idea of the algorithm we ae implementing.
...
The full code for `FindDuplicates` can be found in the <a href={`${
// @ts-ignore
    frontmatter.repositoryUrl}/blob/master/find_duplicates.scala`}>repo</a>.
...
- The type-level code mimics <a href={`${
// @ts-ignore
    frontmatter.repositoryUrl}/blob/master/value_level_find_duplicates.scala`}>value-level</a> code very well. If you were to start from some regular value-level code, translating to a type-level equivalent is fairly straightforward[^rules].
...
To follow along the type-level code, you can find the equivalent value-level code <a href={`${
// @ts-ignore
    frontmatter.repositoryUrl}/blob/master/value_level_error_generation.scala`}>here</a>.
...
The full code for this part can be found in the accompanying <a href={`${
// @ts-ignore
    frontmatter.repositoryUrl}/blob/master/error_generation.scala`}>repo</a>.

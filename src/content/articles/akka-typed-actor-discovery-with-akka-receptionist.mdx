---
category: guide
excerpt: "A common pattern in Akka Typed: discovering how to find actors not explicitly passed around"
publishedDate: 2022-03-22
tags: [akka, akka-typed]
title: "Akka Typed: Actor Discovery with Akka Receptionist"
...
We talk about discovery in the [Akka Typed](/courses/akka-pekko-essentials) course, in the context of routers and work distribution. It's a pretty powerful technique. This article assumes some basic familiarity with typed actors.

If you want the video version, check below:

<YouTube id="iuq-JBUiLpI" />
...
## The Akka Receptionist: Publishing
...
## The Akka Receptionist: Subscribing
...
However, because our Sensor actor is typed with `SensorCommand` and we also need to handle the listing message, we will need a message adapter. We discussed the message adapter technique in [another article](/articles/akka-typed-adapting-messages), so we will use it here. We will need to wrap the listing message into some other `SensorCommand` that we can handle later:
...
We need to finish off the `DataAggregator` and the sensor controller. The data aggregator will keep track of all the readings received so far from all the sensors, using a ["stateless"](/articles/akka-typed-actors-stateful-and-stateless) approach, which we also describe in another article:

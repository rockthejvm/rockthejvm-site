...
We need to finish off the `DataAggregator` and the sensor controller. The data aggregator will keep track of all the readings received so far from all the sensors, using a ["stateless"](akka-typed-actors-stateful-and-stateless) approach, which we also describe in another article:

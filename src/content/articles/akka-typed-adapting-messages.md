---
category: guide
excerpt: Learn how to organize code, messages, domains, and logic in an Akka application with Scala
publishedDate: 2021-03-28
tags: [akka, akka-typed]
title: "Akka Typed: Adapting Messages Explained"
...
The rule of thumb is that _each actor needs to support its own "request" type and nothing else_.
...
This solution is a quick way to ensure that the Checkout actor _is only responsible for messages of type `Checkout.Request`_. Of course, the actual logic of handling the response from the ShoppingCart actor will have to live somewhere, but the responsibility is defined in terms of the declared actor type (watch the `apply` method return type).

...
<NoteAdmonition>
  This notation has nothing to do with the literal types [recently added in
  Scala 3](/articles/scala-3-new-types). The literal types have no relationship
  to each other, so we'll need to model this relationship ourselves.
</NoteAdmonition>

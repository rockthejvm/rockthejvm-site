...
This notation has nothing to do with the literal types [recently added in Scala 3](/articles/scala-3-new-types). The literal types have no relationship to each other, so we'll need to model this relationship ourselves.

:::

## Ordering Numbers at the Type Level
...
## Proving Ordering at the Type Level
...
That in Scala 3.1 (not out yet at the time of writing) you'll be able to say `<[_0, Succ[B]]()` instead of `<[_0, Succ[B]] with {}`, for conciseness.
...
## Testing Type-Level Comparisons
...
- Again, the second `given` can be used, for the same reason. For that, the compiler needs a given of type `<[_1, _3]`.
...
## Extending Type-Level Comparisons

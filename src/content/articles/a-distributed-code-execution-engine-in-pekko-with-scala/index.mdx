...
category: guide
excerpt: "A practical guide to building a distributed system with Scala and Pekko: learn how to run other people's code in a real-world scenario"
publishedDate: 2024-06-14
repositoryUrl: https://github.com/ghurtchu/braindrill
tags: [pekko, pekko-cluster, pekko-http, pekko-stream, scala]
title: A Distributed Code Execution Engine in Pekko with Scala
...
<Tabs syncKey="languages">
    <TabItem label="Scala">
        ```scala
        def americanTime(n: Int): Int =
            val cycle = LazyList.continually(0 to 11).flatten
            cycle(n)
        ```
    </TabItem>
    <TabItem label="Haskell">
        ```haskell
        americanTime :: Int -> Int
        americanTime n = cycle [0..11] !! n
        ```
    </TabItem>
</Tabs>
...
<Tabs syncKey="commands">
    ...
    <TabItem label="pnpm">
        ```shell
        pnpm install software
        ```
    </TabItem>
        <TabItem label="yarn">
        ```shell
        yarn install software
        ```
    </TabItem>
</Tabs>
...
Hey, it's Daniel here. Apache Pekko is great, and Nika has done a great job showcasing its features in a compact project we can explore exhaustively in this article.

:::

:::tip

If you need to get started with Pekko, I've covered Pekko (and Akka) in a comprehensive bundle of courses about [Akka/Pekko Actors](/courses/akka-pekko-essentials), [Akka/Pekko Streams](/courses/akka-pekko-streams), and [Akka/Pekko HTTP](/courses/akka-pekko-http), all of which are used in this article. Check them out if you're interested.

:::

## Project Structure

To make the best of this article, I recommend checking out <a href={frontmatter.repositoryUrl}>the project on GitHub</a> and following the code while reading this article, as many things will make sense along the way.
...
![Project architecture diagram showing a user interacting with a Pekko cluster](images/project-architecture-diagram.png)
...
1. Services: Defines four services - `master`, `worker-1`, `worker-2`, and `worker-3`.
...
1. Running Docker Command: Constructs and runs a Docker command with a 2-second timeout and memory limits.
...
1. Handling Streams: Handles both the standard and error streams from the Docker process.
1. Reading Output: Reads the success and error streams and joins them with the exit code.
1. Deleting File: Schedules the file for deletion to free up memory.
1. Piping Result: Uses `ctx.pipeToSelf` to handle the asynchronous execution result.
...
1. Replying: Sends an `ExecutionSucceeded` message to the requester.
1. Stopping Behavior: Stops the actor.

### `In.ExecutionFailed` case

1. Logging: Logs the failure reason.
1. Replying: Sends an `ExecutionFailed` message to the requester.
1. Stopping Behavior: Stops the actor.
...
1. `src`: Creates a source from an input stream.
1. `run`: Executes a command using `sys.runtime.exec` and returns a future of the process.

### Detailed Workflow

1. Execution Start: When the `Execute` message is received, the actor starts by logging the event.
1. Docker Command: It then constructs and runs the Docker command with specified limits.
1. Stream Handling: Both the success and error streams from the Docker process are handled and read concurrently.
1. Output Management: The output is read and checked against the maximum allowed size.
1. File Deletion: The file is scheduled for deletion to free up memory.
1. Result Handling: The result of the execution (success or failure) is handled and appropriate messages are constructed and sent to the requester.
1. Actor Lifecycle: Depending on the message (`ExecutionSucceeded` or `ExecutionFailed`), the actor stops itself after sending the reply.
...
1. Dynamic Discovery and Registration:
...
1. Robust and Resilient Execution:
...
1. HTTP Server: On the `master` node, it starts an `HTTP` server to accept code execution requests.
1. Configuration: Reads settings for the number of `workers`, `load balancers`, and `HTTP` server details.
1. Worker Router: Each `worker` node creates a pool of ``worker actors using `round-robin` routing.
1. Load-Balancer: The `master` node creates `load balancers` to forward tasks to `worker` routers.
...
For the details, please view the <a href={`${frontmatter.repositoryUrl}/blob/main/README.md`}>README</a> of the original project.
...
1. Docker-out-of-Docker (DooD) Approach:
...
1. Resource Management and Security:

...
As shown in the [higher overview diagram](#higher-overview), after a task is submitted, the worker passes that task by `send()` method to its corresponded task queue through a shared channel called **tx**, which is a *[LinkedBlockingQueue](https://docs.oracle.com/en/java/javase/24/docs/api/java.base/java/util/concurrent/LinkedBlockingQueue.html)* class. The relationship between *Worker* and *[Task Queue](#task-queue)* is 1 to 1.

...
def drunkFlip: Boolean < (Abort[Exception] & Sync) =
  for
    caught <- Random.nextBoolean
    result <- if caught then Random.nextBoolean
              else Abort.fail(new Exception("We dropped the coin!"))
  yield result
```

The type `Boolean < (Abort[Exception] & Sync)` means "a Boolean computation with Abort and Sync effects". The `Random` operations in Kyo return values with the `Sync` effect pending. This is more modular than IO and more compositional than ZIO's three-parameter type. Each effect can be handled independently using effect handlers. Want to run just the error handling? Use `Abort.run`. Want to mock out the randomness for tests? Provide a test handler via `Random.let`.

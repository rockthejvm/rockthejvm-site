...
It's not important which specific effect system we use. The above code wrapped the `getUser` effectful operation with retries, timeouts, and error handling, and we still have an `IO[User]` back.
...
If we want to have the exact list of the effects used, we can use a different approach, using tagless final style with type classes to represent capabilities:

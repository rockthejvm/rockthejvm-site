...
This reads as "given a `Logger` implicitly, produce an `Unit`". The `?=>` arrow indicates that the `Logger` will be provided through Scala's context mechanism, not passed explicitly. We can even summon the instance of the `Logger` inside the function body using `summon[Logger]` instead of having a named parameter:
...
These handlers "run" effects by providing actual implementations. The `Random.run` handler provides a real random capability that uses Scala's `Random` under the hood. The `Raise.either` handler provides an abort capability that uses Scala 3's boundary/break control flow to implement early return on failure. **When we call the program function with an instance of the effect in scope, the effect executes. This is where deferred execution ends and real execution begins:**
...
The `partialResult` still needs a `Raise[String]` effect to run, but the `Random` effect has been handled. It's something like algebraic effects, but without monadic composition.

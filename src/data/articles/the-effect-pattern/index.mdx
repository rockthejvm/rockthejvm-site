...
excerpt: "Explore Scala effect systems from monadic approaches (Cats Effect, ZIO, Kyo) to novel direct-style handlers using context functions, including Ox and YAES"
publishedDate: 2026-02-17
tags: [scala-3, fp, effect-systems]
...
val result2 = result1      // Same value as result1
```

At first glance, `result1` and `result2` always hold the same value. Now, if `drunkFlip()` were referentially transparent, we should be able to substitute `result1` with its definition `drunkFlip()` in the second line without changing the program’s behavior. Let’s try:

```scala 3
val result1 = drunkFlip()  // Say it returns "Heads"
val result2 = drunkFlip()  // A fresh call — might return "Tails" or throw!
```

**The program changed.** Before the substitution, `result2` was guaranteed to equal `result1`. After the substitution, `result2` might be different because each call to `drunkFlip()` can produce a different result or even throw an exception. We replaced equals with equals and got a different program. **The substitution model breaks down, and `drunkFlip()` is not referentially transparent.**
...
Why go through this effort? The benefits are concrete and immediate. Consider referential transparency first. As we said, without the Effect Pattern, our drunk flip breaks substitution. With the Effect Pattern, we get referential transparency back:

```scala 3
val result1 = IO(drunkFlip())  // A description, not an execution
val result2 = result1
// result1 and result2 are identical descriptions
...
It's not important which specific library we use. The above code wrapped the `getUser` effectful operation with retries, timeouts, and error handling, and we still have an `IO[User]` back.
...
If we want to have the exact list of the effects used, we can use a different approach, using tagless final style with type classes to represent capabilities (don't know tagless final? Check out the article [Tagless Final in Scala Quickly Explained](https://rockthejvm.com/articles/tagless-final-in-scala)):
...
The `Raise` and `Random` type classes represent the required effects. They are included in the Cats MTL library. This approach gives us more precision about the effects used, but it adds even more complexity with type classes.
...
The signature `ZIO[Random, String, String]` is remarkably expressive. This operation needs to generate random numbers (the `Random` dependency), might fail with a `String`, and produces a `String` on success. We get more type-level information than Cats Effect's `IO`. The trade-off is that the type is more complex to understand initially.
...
```scala 3
val drunkFlipOnConsole: ZIO[Random & Console, String, Unit] =
...
object Main extends ZIOAppDefault {
  override def run =
    drunkFlipOnConsole.provide(Random.live, Console.live)
}
```

But we're still in monad-land. The `ZIO` type is a monad defined on the type `A`. It's still an über-monad in the sense that we still need to execute everything in a single step.
...
def drunkFlip: String < (Abort[String] & Sync) =
  for {
    caught <- Random.nextBoolean
...
The type `String < (Abort[String] & Sync)` means "a `String` computation with `Abort` and `Sync` effects". The `Random` operations in Kyo return values with the `Sync` effect pending. The `Sync` effect defers any computations that can perform side effects.
...
The type `A` is the result type, and `S` is the stack of effects. The composition of effects is still monadic, In this sense, the `<` is still an uber-type. The difference is that effects are modular and can be handled independently. This is more modular than Cats Effect `IO` and more compositional than ZIO's three-parameter type.
...
When we call `greet("Alice")`, the compiler looks for a `Logger` instance in scope and passes it automatically. This is familiar from Scala 2's implicits, just with cleaner syntax (see [Scala 3: Givens and Implicits Quickly Explained](https://rockthejvm.com/articles/scala-3-givens-and-implicits) for further details).
...
```scala 3
val partialResult: Raise[String] ?=> String = Random.run {
  drunkFlip
}
```

The `partialResult` still needs a `Raise[String]` capability to run, but the `Random` effect has been handled. It's something like algebraic effects.
...
The `Raise` handler converts failures to `Either`, but we could write a handler that logs errors, or retries, or does something else entirely. **This flexibility is the power of the effect pattern. We separate the description of effects from their interpretation.**
...
### What Kind of Deferral Is This?

Now that we've seen how direct-style effects work, we need to be honest about what kind of deferral we're actually getting. As we said earlier, the Effect Pattern rests on deferred execution. But not all deferral is created equal, and the difference matters.

In monadic systems, `IO[A]` is an **inert data structure** — a description of a program that a runtime interprets. The program itself is a value we can store, retry, race, and compose. We can pass it around, inspect it, and run it multiple times. This is the strongest form of deferred execution: **programs as data**.

In direct-style, the effectful program is a **context function** (`C ?=> A`). It genuinely won't execute until a handler supplies the capability. We can store it, pass it around, and choose when to execute it. This is real deferral — but it's not the same as programs-as-data.

The key difference: **once we provide the context and the function body executes, effects fire eagerly.** There's no intermediate data structure a runtime can inspect, optimize, or re-execute. Let's see an example to make this concrete:

```scala 3
// Monadic: program is a reusable data structure (pseudocode)
val program: IO[Int] = IO(Random.nextInt())
program.retryN(3)              // re-invokes the computation three times
program.race(program)          // two independent executions

// Direct-style: program is a function, not data
val program: Random ?=> Int = Random.nextInt
// Can't retry or race without wrapping in explicit lambdas
```

In the monadic version, `program` is a value that describes what to do. The runtime can execute that description as many times as it wants. In the direct-style version, `program` is a function waiting for its context. Once we provide that context, the body runs once and produces a result. If we want to retry or race, we need to explicitly wrap the computation in lambdas or reconstruct it. For example, we can achieve retries in direct-style, but we need to be explicit about it:

```scala 3
// Direct-style: retries require an explicit loop
def retry[A](n: Int)(program: => A): A =
  try program
  catch
    case e: Exception if n > 0 => retry(n - 1)(program)

// Usage
retry(3) {
  Random.run { Random.nextInt }
}
```

Notice that the caller must pass the computation as a **by-name parameter** (`=> A`) so that each retry re-executes the body. In monadic style, this comes for free because `IO[A]` is already a description we can re-run. In direct-style, we have to reconstruct the deferral manually.

### But Isn't This Just `Future`?

At this point, a reasonable objection arises: if direct-style effects lose referential transparency and don't give us programs-as-data, aren't we just reinventing `Future` with an implicit `ExecutionContext`? The surface similarity is real: both approaches thread an implicit parameter through our code. But the comparison breaks down under closer inspection.

The critical difference is **eagerness**. `Future` is eager on construction. The moment we create a `Future`, its body starts executing immediately:

```scala 3
import scala.concurrent.Future
import scala.concurrent.ExecutionContext.Implicits.global

val f: Future[Int] = Future(expensiveComputation()) // Already running!
val g: Future[Int] = f // g shares f's result, not the computation
```

A context function, by contrast, **does nothing until a handler provides the capability**. The computation is genuinely deferred:

```scala 3
val program: Random ?=> Int = Random.nextInt // Nothing happens yet
// Only executes when we provide the handler:
Random.run { program } // Now it runs
```

The second difference is **accidental concurrency**. With `Future`, composing two independent operations with `flatMap` might run them in parallel depending on where they're created. This leads to subtle, hard-to-reproduce bugs. Direct-style sequential code runs sequentially. If we want parallelism, we must explicitly request it.

The third difference is **memoization**. `Future` memoizes its result: once it completes, it caches the value. Calling `.map` on the same `Future` twice gives the same result. Context functions re-execute every time we provide the context, each invocation is a fresh computation.

In short, `Future` is eager, accidentally concurrent, and memoized. Direct-style is deferred, sequential, and re-executable. They share only the surface syntax of implicit parameters.

In other words, **direct-style sits between `Future` (no deferral at all) and `IO` (full programs-as-data).** It fixes `Future`'s worst problems while trading away `IO`'s strongest guarantees. As we'll see in the trade-offs section, this positioning has concrete consequences for what we can and can't do.

## Direct-Style versus Monadic: Trade-offs and Honest Assessment
...
As you can see, `genRand` is now a `def` rather than a `val`, so each time we call it, we get a fresh random value. It's not referential transparency, since we're not representing programs through values, but we're just calling functions. Moreover, we have to remember to use `def` for computations that need re-evaluation.
...
This connects to a deeper point. As we discussed in the "What Kind of Deferral Is This?" section, monadic `IO` gives us **programs as data**: we can retry, race, and compose program descriptions freely. In direct-style, the computation is a function, not data. In terms of continuations, monadic `IO` gives us effectively _multi-shot continuations_ (the runtime can invoke the same program description multiple times), while direct-style with context functions gives us _one-shot continuations_ (each computation runs exactly once). As we'll see shortly, this distinction determines which effects we can express.
...
If we return to a setup where we use an imperative style, we lose the ability to treat our programs as values and thus the full power to manipulate their control flow. In fact, **the direct-style approach we presented can't express all possible effects** (e.g., the [`Choice` effect](https://getkyo.io/#/?id=choice-exploratory-branching)). This is because we can't express all types of continuations with context functions. (Yes, it's all a matter of continuations). Context functions can express a subset of continuations called one-shot continuations.
...
We also gain **freedom from accidental concurrency**. This was arguably the biggest practical problem with `Future`-based designs: two `Future` values created independently could run in parallel without the developer intending it. In direct-style, sequential code runs sequentially. Parallelism must be explicitly requested through constructs like structured concurrency. **What we write is what we get** and we should not expect hidden concurrency surprises.
...
As we can see, direct-style occupies a middle ground. It avoids the pitfalls of `Future` while being more approachable than full monadic `IO`, at the cost of some guarantees that monadic systems provide.
...
We've built our own minimal effect system to understand the principles. But we don't have to roll our own in production. The Scala ecosystem has direct-style libraries that provide implementations. Two notable ones are: [Ox](https://ox.softwaremill.com/latest/) and [YAES](https://blog.rcard.in/yaes/).

...
The code in this article will use many different libraries to illustrate the different approaches. To follow along, ensure you have Scala 3 set up in your development environment. Moreover, let's add the following dependencies to our `build.sbt` file:
...
These are just interfaces. Given to a context function, **they represent "the permission to perform the effect".** Functions that need these effect declare them with `using` parameters. Without the a concrete instance of the effect type in scope, the function can't compile. The type system enforces that effects can only happen when we have permission.
...
Look at that code. It reads like imperative code: check a condition, abort if needed, otherwise flip the coin. But notice the `using` parameters in the signature. **This function requires instances of the Random and Raise effect to run.** Without them in scope, calling this function is a compile error. This is how we achieve deferred execution. The function body describes what to do, but it can't do anything until someone provides the effects.
...
The `Raise.either` handler uses Scala 3's `boundary` and `break` for control flow. Think of `boundary` as defining a labeled scope and `break` as a way to exit that scope early with a value. When `raise.raise` is called, `break(Left(error))` immediately exits the boundary block and returns `Left(error)`. This is how we convert an abortion into an `Either` without using exceptions. If the program succeeds, we wrap the result in `Right`.
...
The order of handlers matters. It's not the case in our simple example, but in more complex scenarios, the order in which we handle effects can change behavior.
...
Notice each handler can interpret effects differently. The `Random` handler uses real randomness, but we could write a test handler that returns fixed values:
...
Both work. Both are (almost) safe. The difference is in how much conceptual overhead they impose on the developer.
...
Fair enough, let's examine what we lose and what we gain.
...
When the handler for `Random` is provided, the `genRand` value is evaluated eagerly. Each time we reference `genRand`, we get the same value. This breaks referential transparency at the value level. We can't replace `genRand` with `Random.nextBoolean` without changing behavior, because it only gets evaluated once.
...
As you can see, now `genRand` is a `def` and not a `val`, so each time we call it, we get a fresh random value. This restores referential transparency at the cost of some verbosity. We have to remember to use `def` for computations that need to be re-evaluated.
...
However, do we really need referential transparency? Referential transparency is a core concept in models of programming where we can express programs as values. In this setup, we need to be able to substitute values freely without changing program behavior. This is crucial for reasoning about code, composing it, and ensuring correctness. In particular, it's crucial to manage the control flow of effects safely.

If we return to a set up where we use an imperative style, we lose the ability to treat our programs as values, and so to have the full power to manipulate the control flow of a program. In fact, the direct-style approach we presented can't express all the possible effects (i.e., the [`Choice` effect](https://getkyo.io/#/?id=choice-exploratory-branching)). It's due to the fact that we can't express all the type of continuations with context functions. (Yes, it's all a matter of continuations). Context functions can express a subset of continuations, called one-shot continuation. However, this is far beyond the scope of this article.

In many practical applications, the effect that we can express with direct-style handlers are sufficient. Most applications don't need the full power of continuations or advanced control flow. They just need to manage side effects like randomness, I/O, and error handling in a safe and composable way. For these common cases, direct-style handlers provide a simpler and more approachable way to achieve the Effect Pattern.
...
We've explored two approaches to the Effect Pattern in Scala. Monadic systems like Cats Effect, ZIO, and Kyo use explicit effect types and `flatMap` composition. They're powerful, battle-tested, and widely used in production systems around the world. The learning curve is steep, but the payoff is robust effect management with rich combinators for composition, error handling, and concurrency. Direct-style systems use context functions to achieve the same goals with different syntax. They're more readable, more approachable, but they trade away some referential transparency at the value level. Both achieve the same fundamental goals: type tracking and deferred execution. Despite this, the difference is in syntax and cognitive load. In other words, both let us manage side effects safely. They just express that safety differently.

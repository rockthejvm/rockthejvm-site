...
def drunkFlip: ZIO[Any, Exception, Boolean] =
  for {
    caught <- Random.nextBoolean
    result <- if (caught) Random.nextBoolean
              else ZIO.fail(new Exception("We dropped the coin!"))
  } yield result
```

The signature `ZIO[Any, Exception, Boolean]` is remarkably expressive. This operation needs no special dependencies (`Any` means no requirements), might fail with an `Exception`, and produces a `Boolean` on success. We get more type-level information than Cats Effect's `IO`. The trade-off is that the type is more complex to understand initially.

But we're still in monad-land. The for-comprehension is still there. The cognitive load of `flatMap` chains remains. ZIO provides a richer algebra for working with effects than Cats Effect, avoiding some of the monad transformer problems, but the fundamental monadic style persists. The power is undeniable, but so is the learning curve for newcomers to functional programming.

### Kyo: Algebraic Effects (Still Monadic)

Kyo takes a different approach that introduces **algebraic effects** to Scala. Instead of one über effect type, effects are composed using type-level operators. **You list effects separately and can handle them separately.** This is more modular than the über effect approach. Want randomness and error handling? Compose `Random` and `Abort`. Need different effects elsewhere? Use a different composition.
...
def drunkFlip: Boolean < (Abort[Exception] & Random) =
  for {
    caught <- Random.nextBoolean
    result <- if (caught) Random.nextBoolean
              else Abort.fail(new Exception("We dropped the coin!"))
  } yield result
```

The type `Boolean < (Abort[Exception] & Random)` means "a Boolean computation with Abort and Random effects". This is more modular than IO and more compositional than ZIO's three-parameter type. Each effect can be handled independently using effect handlers. Want to run just the error handling? Use `Abort.run`. Want to mock out the randomness for tests? Provide a test handler for `Random`.
...
All three approaches solve the Effect Pattern beautifully. They track effects in types, giving us compile-time safety. They defer execution until we're ready, separating description from action. They provide rich algebras for composing effects. They're used in production systems around the world and have proven their worth. **But they share a challenge that we can't ignore: monadic composition has a steep learning curve.**
...
The magic is **context functions**, a Scala 3 feature that changes how we think about effects. **Instead of wrapping effects in a monad, we write functions that require permission to run.** That permission is a **capability** passed implicitly using `using` parameters. Think of it like this: monadic style says "here's a wrapped effect, run it by calling methods on it". Direct-style says "here's code that looks normal, but it can't run without permission". The permission is the capability, and the compiler ensures you can't execute effectful code without it.

A context function has type `Capability ?=> A`, which means "give me a Capability implicitly and I'll give you an A". The `?=>` syntax indicates that the capability is passed using Scala's context parameter mechanism. **The Scala compiler threads this capability through our code automatically.** Nothing actually executes until we provide the capability explicitly. This is deferred execution without wrapping in `IO` or using `flatMap`. The deferral comes from requiring a capability that hasn't been provided yet.

Here's the crucial insight: when a function needs a capability through a `using` parameter, and we call that function inside another context that also requires the same capability, the compiler passes it automatically. The capability flows through our code invisibly. When we write `val caught = Random.nextBoolean` inside a function that has `using Random`, the compiler sees this and thinks "aha, Random.nextBoolean needs a Random capability, and we have one in scope, so I'll pass it along automatically". We don't need to write `Random.nextBoolean(using random)` explicitly. The compiler handles the threading for us.
